...
> 아래 인용은 읽기 좋게 들여쓰기를 정돈한 것이다. **리포의 `firestore.rules`에는 콘솔 원문이 공백 하나까지 그대로** 들어 있다 (feedback 블록의 3칸 들여쓰기, 빈 줄의 후행 공백 포함). 그 파일을 이 인용에 맞춰 "정리"하지 말 것 — 무변화 배포를 확인하기 전까지는 원문과의 동일성이 유일한 안전장치다.
