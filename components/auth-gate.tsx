...
export function AuthGate({ children }: Props) {
  ...
  // 1. 첫 접속 시 "다시 보지 않기" 체크 안 한 유저에게 튜토리얼 자동 노출 (로그인 전)
  useEffect(() => {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-6 px-4 relative">
        ...
        <div className="flex flex-col items-center gap-3 w-full max-w-sm">
          <button
            onClick={handleLogin}
            className="w-full flex items-center justify-center gap-2 px-6 py-3 bg-white text-gray-700 border border-gray-300 rounded-xl shadow-sm hover:shadow-md transition-all text-sm font-medium"
          >
        ...
        {/* 첫 접속 시 켜지는 튜토리얼 라이트박스 팝업 */}
        {showOnboarding && (
          <OnboardingModal
            onClose={() => setShowOnboarding(false)}
            onSelectTab={() => {}} // 로그인 전이므로 탭 이동 스킵
          />
        )}
      </div>
    )
